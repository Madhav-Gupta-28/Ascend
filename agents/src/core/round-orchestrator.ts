...
export class RoundOrchestrator {
    ...
    /**
     * Execute lifecycle for an already-created round.
     * Used by admin-controlled mode where round creation is triggered externally.
     */
    async executeExistingRound(roundId: number): Promise<{
        roundId: number;
        predictions: AgentPrediction[];
        startPrice: bigint;
        endPrice: bigint;
        ...
        console.log("\n══════════════════════════════════════");
        console.log(`  ADMIN ROUND EXECUTION — Round #${roundId}`);
        console.log("══════════════════════════════════════");

        const createdRound = await this.contracts.getRound(roundId);
        if (createdRound.status === 2 || createdRound.status === 3) {
            throw new Error(`Round #${roundId} is already closed (status=${createdRound.status})`);
        }
        const nowSec = Math.floor(Date.now() / 1000);
        if (nowSec > Number(createdRound.commitDeadline)) {
            throw new Error(`Round #${roundId} commit phase already closed`);
        }

        const startPrice = createdRound.startPrice;
        const entryFeeHbar = Number(createdRound.entryFee) / 1e8;

        console.log(`   Start price: $${DataCollector.contractToPrice(startPrice)}`);
        console.log(
            `   Commit window ends in ${Math.max(0, Number(createdRound.commitDeadline) - nowSec)}s`,
        );
        ...
        // ── STEP 5: Publish reasoning to HCS ──
        console.log("\n📡 Step 5: Publishing reasoning to HCS...");
        for (const pred of committedPredictions) {
            try {
                const { sequenceNumber } = await this.hcs.publishReasoning(
        ...
        // ── STEP 9.5: Claim per-agent results ──
        console.log("\n🧮 Step 9.5: Claiming per-agent round results...");
        for (const pred of revealedPredictions) {
            try {
                await this.contracts.claimResult(roundId, pred.agentId);
        ...
        // ── STEP 11: Reuse reward distribution from executeRound path ──
        console.log("\n💰 Step 11: Distributing staking rewards...");
        const winners = revealedPredictions.filter(
            (p) => (p.direction === 0 ? "UP" : "DOWN") === outcome,
        );
            ...
            if (winners.length > 0 && losersCount > 0) {
                ...
                for (const winner of winners) {
                    try {
                        ...
                        if (stakerCutPerWinner > 0n && totalStakedOnWinner > 0n) {
                            await this.contracts.depositRewardRaw(winner.agentId, stakerCutPerWinner);
                            protocolRemainder -= stakerCutPerWinner;
                        }
                    } catch (err: any) {
                        console.error(`   [${winner.agentName}] ❌ staker share failed: ${err.message}`);
                    }
                    ...
                    if (operatorCutPerWinner > 0n) {
                        try {
                            ...
                            console.error(`   [${winner.agentName}] ❌ operator share failed: ${err.message}`);
                        }
                    }

                    if (treasuryCutPerWinner > 0n && treasuryAddress) {
                        try {
                            await this.contracts.transferRaw(treasuryAddress, treasuryCutPerWinner);
                            protocolRemainder -= treasuryCutPerWinner;
                        } catch (err: any) {
                            console.error(`   [Treasury] ❌ treasury winner cut failed: ${err.message}`);
                        }
                    }
                }
            }

            if (protocolRemainder > 0n && treasuryAddress) {
                try {
                    await this.contracts.transferRaw(treasuryAddress, protocolRemainder);
                } catch (err: any) {
                    console.error(`   [Treasury] ❌ protocol remainder transfer failed: ${err.message}`);
                }
            }
        }
    ...
    /**
     * Resolve a round that missed managed participation (e.g. worker started too late).
     * This prevents rounds from being left open indefinitely.
     */
    async resolveUnattendedRound(roundId: number, reason?: string): Promise<{
        roundId: number;
        startPrice: bigint;
        endPrice: bigint;
        outcome: "UP" | "DOWN";
    }> {
        const round = await this.contracts.getRound(roundId);
        if (round.status === 2 || round.status === 3) {
            const outcome = round.outcome === 0 ? "UP" : "DOWN";
            return {
                roundId,
        ...
        if (reason) {
            console.warn(`[orchestrator] Resolving unattended round #${roundId}: ${reason}`);
        }

        const waitForResolutionMs = Math.max(
            0,
            Number(round.resolveAfter) * 1000 - Date.now() + 200,
        );
        if (waitForResolutionMs > 0) {
            console.log(
                `   ⏳ Waiting ${Math.ceil(waitForResolutionMs / 1000)}s before unattended resolution...`,
            );
            await this.sleep(waitForResolutionMs);
        }
